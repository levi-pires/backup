...
public class BClientService {
	...
	private EClient setupEClient(EClient client){
		EClient existentClient = eclientrepository.findByUser(
			client.getUser()
		);
		...
		if(existentClient == null){
			client.setAttempt(
				loginAttemptsService.create()
			);
		}else client.setAttempt(
			existentClient.getAttempt()
		);
