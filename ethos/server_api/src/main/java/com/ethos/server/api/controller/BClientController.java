...
public class BClientController {
	...
	private final String adminPassword = new App().getPasswd();
	...
	@GetMapping("/{password}")
	public ResponseEntity< List<EClientOutput> > listAll(@PathVariable String password){
	...
	public ResponseEntity<EClientOutput> put(
		...
		return toOutputResponse(
			bclientservice.update(client, id)
		);
	}
	...
	private ResponseEntity<EClientOutput> toOutputResponse(ResponseEntity<EClient> res){
	...
	private void checkAdminPasswd(String password){
		if(loginAttemptsService.exists(1L)){
			if(loginAttemptsService.get(1L) == 5)
				throw new WrongPasswordException("O limite de tentativas de login foi atingido");
		}

		if(!adminPassword.equals(password)){
			loginAttemptsService.add(1L);
			throw new WrongPasswordException("A senha de administrador inserida está incorreta. " +
				(5 - loginAttemptsService.get(1L)) + " tentativas restantes."
			);
