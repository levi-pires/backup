...
public class EClientLoginController {
	...
	public ResponseEntity<EClientOutput> eLogin(
		...
		if(loginAttemptService.get(attemptId) == 4)
			throw new WrongPasswordException("O limite de tentativas do login foi atingido.");
		
		String key = digestPassword.digest(userPassword);
		
		if(!key.equals(client.getPassword())){
			loginAttemptService.add(attemptId);
			throw new WrongPasswordException("Senha de usuário incorreta. Você tem " 
				+ (5 - loginAttemptService.get(attemptId)) + " tentativas restantes."
			);
		}
	
		if(loginAttemptService.get(attemptId) > 0)
			loginAttemptService.reset(attemptId);
	...
	private ResponseEntity<EClientOutput> toOutputResponse(ResponseEntity<EClient> res){
		return ResponseEntity
			...
			.body(
				modelmapper.map(
					res.getBody(), EClientOutput.class
				)
			);
	}
